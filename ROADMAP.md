...
> Cocher `[x]` quand fait. Réorganiser les priorités selon besoins.
...
### Sécurité
- [x] **Vérification signature webhook Stripe** (HMAC SHA-256 via Web Crypto API)
- [x] **Rejet des webhooks Stripe en mode test** en production (anti-faux Premium)
...
### RGPD / Conformité
- [x] **Suppression de compte** (endpoint `/account/delete` : Stripe cancel + KV purge + Firebase Auth delete)
- [x] **Export RGPD des données utilisateur** (JSON download depuis Mon Profil)
...
- [x] **Page "Mon Profil"** complète avec hero header + 5 cards
- [x] **Avatar utilisateur** uploadable (resize 128×128, stockage Firebase RTDB)
- [x] **Menu déroulant navbar** style Google (avatar + sous-menu + déconnexion)
- [x] **Bouton Support** (?) dans navbar avec modal + formulaire de contact
- [x] **Email support** envoyé à `natquinson@gmail.com` via Brevo (rate-limited 5/h)
- [x] **Customer Portal Stripe** (gestion abonnement, factures, annulation)
- [x] **Reset password** depuis Mon Profil
- [x] **Affichage statut abonnement** (Free / Premium mensuel / annuel + date renouvellement)
- [x] **Suppression boutons théme/langue** du topbar (maintenant dans menu profil)

---

## 📊 Priorité 3 — Analytics & Observabilité (DONE ✅)

- [x] **Intégration native Google Analytics 4** dans l'admin Kairos (Data API)
  - JWT signing RS256 + token caché en KV
  - KPIs (utilisateurs, sessions, page views, bounce, durée)
  - Top pages + sources de trafic
- [x] **Tableau de bord admin complet** (Phase A-F : users, subs, traffic, DB, jobs)
