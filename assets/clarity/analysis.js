...
(() => {
  ...
  function render(host, company = {}) {
    ...
    function addMetric(key, config) { if(window.KairosUI?.lang==='en'){const pair=window.KairosMetricEnglish?.[key];if(pair){config.description=pair[0];config.formula=pair[1];}else if(key.startsWith('criterion')){config.description='The displayed condition is evaluated only when its source data is available.';config.formula='One point when the criterion is verified.';}else if(key.startsWith('bucket-')){config.description='The number of analyst opinions in this rating category.';config.formula='Count supplied by the consensus provider.';}} metrics.set(key,config); return key; }
    ...
    criteria.forEach((criterion,index)=>addMetric(`criterion${index}`,{label:String(criterion.label),formatted:criterion.pass===true?'Critère rempli':criterion.pass===false?'Critère non rempli':'Donnée indisponible',description:criterion.pass===true?'La condition indiquée est remplie dans les données de les données disponibles.':criterion.pass===false?'La condition indiquée n’est pas remplie dans les données de les données disponibles.':'Les données disponibles ne permettent pas de vérifier cette condition.',formula:'Ce contrôle contribue à hauteur d’un point au compteur lorsqu’il est rempli. Il ne modifie pas directement le radar.',unit:'text'}));
