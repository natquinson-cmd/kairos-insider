...
(function(root,factory){const api=factory();if(typeof module==='object')module.exports=api;else root.KairosAdapter=api;})(typeof window==='object'?window:this,()=>{
  ...
  function stock(d){
    ...
    const fundamentals={...f,trailingPE:number(f.peRatio),forwardPE:number(f.forwardPE),priceSales:number(f.psRatio),priceBook:number(f.pbRatio),priceFcf:number(f.pfcf),evEbitda:number(f.evEbitda),grossMargin:metric(d.margins?.gross),operatingMargin:metric(d.margins?.operating),netMargin:metric(d.margins?.profit),roe:metric(d.returns?.roe),roa:metric(d.returns?.roa),roic:metric(d.returns?.roic),currentRatio:metric(d.financialPosition?.currentRatio),quickRatio:metric(d.financialPosition?.quickRatio),debtEquity:metric(d.financialPosition?.debtEquity)};
