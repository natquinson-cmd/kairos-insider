...
const JOB_REGISTRY = [
  ...
  {
    id: 'realtime-30min',
    name: 'Refresh 1h (13D + AMF + BaFin + seuils EU)',
    cron: '0 * * * *',
    type: 'github-actions',
    ...
    description: 'Refresh haute volatilité toutes les heures à :00. 4 jobs en parallèle : (1) SEC 13D/A activistes + rebuild indexes filer/EU-UK, (2) AMF FR déclarations dirigeants, (3) BaFin DE Directors\' Dealings, (4) seuils EU (AMF + BaFin + AFM NL via Playwright stealth). Latence ~10 min total grâce à la parallélisation. Passe de 30 min à 1h le 13 mai 2026 pour réduire la pression quota GH Actions (80% des slots droppés en 30 min cadence).',
  },
  {
    id: 'form4-realtime',
    name: 'Realtime Form 4 1h (insiders US)',
    cron: '30 * * * *',
    type: 'github-actions',
    ...
    description: 'Refresh Form 4 SEC (insiders US) toutes les heures à :30 (décalé du realtime-30min à :00). Utilise le proxy CF Worker /api/admin/sec-proxy pour contourner le rate-limit des IPs GitHub Actions (fix mai 2026 après data figée 6 jours). Dedup adsh : skip XML fetch des filings déjà indexés (~95% en steady state) -> run de 30-60s vs 12-15 min sans dedup. Split SEC vs non-SEC (BaFin/AMF preserves intacts). Push D1 INSERT OR IGNORE pour Top Insiders ROI history. Passe de 30 min à 1h le 13 mai 2026 pour réduire la pression quota GH Actions.',
  },
  {
    id: 'backup',
