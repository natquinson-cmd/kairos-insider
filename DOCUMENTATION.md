...
| `netflow:{days}:{direction}:{minValue}:{limit}` | Flux net insider par ticker (onglet "Flux net" - period configurable 7j/30j/90j) | 15 min | D1 query cache |
...
> **⚠️ Dedup transaction économique (doublons multi-déclarants)** — Une même opération peut être déclarée par **plusieurs entités liées** (chaîne de détention bénéficiaire). Ex `$SYM` : SoftBank vend Symbotic via `SVF Sponsor III (DE) LLC` (détenteur direct) **et** `SOFTBANK GROUP CORP.` (parent), deux Form 4 SEC distincts (accessions/CIK différents) pour la même vente → comptée 2× (flux net doublé, initiés gonflés). La dedup par `accession+cik+insider` ne les voit pas. **Clé de dedup économique** : `ticker + date transaction + sens + nb titres + montant exact (au cent)`. Appliquée à **3 niveaux** : `merge-sources.py` (source → KV + D1 au prochain run), `aggregateInsiders` (carte initiale, read-time) et `handleHistoryInsider` (refresh D1, read-time, car D1 cumulatif). **Garde-fou** : seules les opérations chiffrées (`shares>0 ET value>0`) sont dédupliquées.
